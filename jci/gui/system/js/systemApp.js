...
systemApp.prototype.appInit = function()
{
	...
	//Show timed SBN Voice Recognition Loading
        ...
        // Indicates Available / Unavailable for WCP, communication items, entertainment items, navi, and settings.
        "StatusMenuCarPlayWifi"       : this._StatusMenuCarPlayWifiMsgHandler.bind(this),

        // Add or remove certain menu items.
        "StatusMenuVisibleCarPlay"    : this._StatusMenuVisibleCarPlayMsgHandler.bind(this),

        // Add or remove certain menu items.
        "StatusMenuVisibleCarPlayWifi": this._StatusMenuVisibleCarPlayWifiMsgHandler.bind(this),

        // Add or remove certain menu items.
        "StatusMenuVisibleAndroidAuto": this._StatusMenuVisibleAndroidAutoMsgHandler.bind(this),
    };
    //@formatter:on
...
systemApp.prototype._initEntertainmentDataList = function()
{
    ...
    items.push(
        ...
        { appData : { appName : 'carplay',        isVisible : false, audioSourceId : 'CarPlayWifi',  mmuiEvent : 'SelectCarPlayWifi'  }, text1Id : 'CarPlay',        disabled : true,   itemStyle : 'style01',   hasCaret: false },
...
{
    log.info("Received StatusMenu message: " + msg.params.payload.statusMenu.appName + " " + msg.params.payload.statusMenu.appStatus);
...
{
    log.info("Received StatusMenuVisible message: ");
    if (msg.params && msg.params.payload)
    {
        ...
        for (var i = 0; i < this._masterEntertainmentDataList.items.length; ++i)
        {
                ...
                && this._masterEntertainmentDataList.items[i].appData.isVisible != isVisible)
            {
                ...
                {
                    log.debug("CarPlay of AndroidAuto, nothing to do");
                }
            }
        }
...
systemApp.prototype._updateEntertainmentNowPlayingIcon = function(audioSource, isPlaying, dataList)
{
    ...
    this._verifyCarPlayWifiIcon(audioSource, isPlaying, dataList);
};
...
systemApp.prototype._StatusUpdateAudioSourceMsgHandler = function(msg)
{
    if (msg.params && msg.params.payload)
    {
        ...
        if (msg.params.payload.statusAudio.audioSource !== "CarPlayWifi")
        {
            audioSourceApp = msg.params.payload.statusAudio.audioSource;
        }
        else
        {
            audioSourceApp = "CarPlay";
        }
        ...
        this._updateApplicationsNowPlayingIcon(audioSourceApp, isPlaying, this._masterApplicationDataList);

        // Update current context if needed
        if (this._currentContext && this._currentContext.ctxtId === "Applications" && this._currentContextTemplate)
        {
            var list = this._currentContextTemplate.list2Ctrl;
            this._updateApplicationsNowPlayingIcon(audioSourceApp, isPlaying, list.dataList);
            list.updateItems(0, list.dataList.items.length - 1);
        }
    }
};

systemApp.prototype._verifyCarPlayWifiIcon = function(audioSource, isPlaying, dataList)
{
    var carPlayWifiIndex = -1;

    for (var i = 0; i < dataList.items.length; ++i)
    {
        if (this._isCarPlayWifiMenu(dataList.items[i]))
        {
            carPlayWifiIndex = i;
            break;
        }
    ...
    log.debug("_verifyCarPlayWifiIcon() : Index of CarPlay Menu for Wifi : " + carPlayWifiIndex);

    if (-1 === carPlayWifiIndex)
    {
        return; // CarPlay Menu for Wifi is NOT Exist. no need to verify.
    }

    // Need to Fix index of Speeker Icon
    for (var i = 0; i < dataList.items.length; ++i)
    {
        if (((audioSource === "CarPlay") && (dataList.items[i].appData.audioSourceId === "CarPlay" || dataList.items[i].appData.audioSourceId === "CarPlayWifi")) ||
            ((audioSource === "CarPlayWifi") && (dataList.items[i].appData.audioSourceId === audioSource)))
        {
            dataList.items[i].image1 = ""; // anyway, Remove speeker Icon from USB CarPlay menu.

            if (isPlaying)
            {
                // Set Speeker Icon to Wifi CarPlay menu.
                dataList.items[carPlayWifiIndex].image1 = "common/images/icons/IcnListEntNowPlaying_En.png";
            }
            else
            {
                // UnSet Speeker Icon to Wifi CarPlay menu.
                dataList.items[carPlayWifiIndex].image1 = "";
            }

            log.info("_verifyCarPlayWifiIcon() : Speeker Icon of Wifi CarPlay menu = " + dataList.items[carPlayWifiIndex].image1);
            break;
        }
    }
...
systemApp.prototype._isCarPlayWifiMenu = function(item)
{
    var isCarPlayWifiMenu = false;

    if ((item.appData.appName == 'carplay') && (item.appData.audioSourceId == 'CarPlayWifi') && item.appData.isVisible)
    {
        isCarPlayWifiMenu = true;
    }

    log.debug("_isCarPlayWifiMenu() : " + isCarPlayWifiMenu);
    return isCarPlayWifiMenu;
};
...
systemApp.prototype._readySourceReconnect = function()
{
        ...
        this._currentContextTemplate.dialog3Ctrl)
    {
        switch (this._currentContext.params.payload.audioSource)
        {
            ...
            case "CarPlay":
                this._currentContextTemplate.dialog3Ctrl.setText1Id("ReconnectingAC");
                break;
...
systemApp.prototype._readySourceReconnectFailed = function()
{
        ...
        this._currentContextTemplate.dialog3Ctrl)
    {
        switch (this._currentContext.params.payload.audioSource)
        {
            ...
            case "CarPlay":
                this._currentContextTemplate.dialog3Ctrl.setText1Id("UnableReconnectAC");
                break;
...
systemApp.prototype._updateApplicationStructure = function(vehicleConfiguration)
{
    if(vehicleConfiguration === "Old")
    {
        ...
        this._applicationsCtxtWiseAppNames = {
          ...
          "Applications"            : ["xmdata","hdtrafficimage","idm","ecoenergy","driverid","warnguide","schedmaint","vehicleStatus","vdt_settings","vdt","carplay","androidauto"],
    ...
    else
    {
        ...
        this._applicationsCtxtWiseAppNames = {
          //Context Name :  // [appName,appName....]
          "Applications"            : ["xmdata","hdtrafficimage","idm","ecoenergy","driverid","vehicleStatus","vdt_settings","vdt","carplay","androidauto"],
...
systemApp.prototype._StatusMenuCarPlayWifiMsgHandler = function(msg)
{
    if (msg.params && msg.params.payload)
    {
        log.info("Received StatusMenuCarPlayWifi message: " + msg.params.payload.appName +
                  " " + msg.params.payload.appStatus);

        // Update menu items associated with the given appName of the message. Menu items can appear
        // in several different contexts or appear multiple times: this function searches all those places.

        var appName = msg.params.payload.appName;
        var isDisabled = msg.params.payload.appStatus !== "Available";
        var deviceName = msg.params.payload.deviceName;
        ...
        if (appName === "carplay")
        {
            var mmuiEvent = "SelectCarPlayWifi";
            for (var i = 0; i < this._masterEntertainmentDataList.items.length; ++i)
            {
                if (this._masterEntertainmentDataList.items[i].appData.mmuiEvent === mmuiEvent)
                {
                    // display or not connected carplay deviceName
                    this._displayDeviceNameEntCarPlayMenu(this._masterEntertainmentDataList.items[i], !isDisabled, deviceName);
                    break;
                }
                else
                {
                    log.debug("appName : " + this._masterEntertainmentDataList.items[i].appData.appName + " is not target.");
                }
            }
            // Update carplay icon of HomeScreen
            ...
            {
                log.debug("context is not Entertainment. nothingto do.");
            }
        }
        else
        {
            log.warn("appName is not carplpay. nothing to do.");
        }
    }
    else
    {
        log.error("msg.params is NULL");
    }
};

systemApp.prototype._StatusMenuVisibleCarPlayMsgHandler = function(msg)
{
    if (msg.params && msg.params.payload)
    {
        log.info("Received StatusMenuVisibleCarPlay message: " +
        msg.params.payload.appName + " " + msg.params.payload.state + " " + msg.params.payload.deviceName);
        ...
        if (appName === "carplay")
        {
            var isDisabled = msg.params.payload.state !== "Visible";

            this._enableEntertainmentListItemCarPlay(isDisabled, deviceName);
            // Update carplay icon of HomeScreen
            this._setRemoteUIButton(appName, !isDisabled);
                    ...
                    case "Entertainment":
                        if (this._currentContextTemplate && this._currentContextTemplate.list2Ctrl)
                        {
                            ...
                            for (var i = 0; i < dataList.items.length; ++i)
                            {
                                var updateListForCarPlay = false;
                                // change only for entertainment list // make better change later
                                if(dataList.items[i].text1Id == 'USB1CarPlay' || dataList.items[i].text1Id == 'UsbAudioA')
                                {
                                    updateListForCarPlay = true;

                                    var CPIndex = this._USB1EntIndex-2;   //Entertainment list index of USB CarPlay
                                    var CPWifiIndex = CPIndex-1;          //Entertainment List index of Wifi CarPlay
                                    ...
                                    if (isDisabled)
                                    {
                                        ...
                                        // retain usb state
                                        isDisabled = this._masterEntertainmentDataList.items[this._USB1EntIndex].disabled;
                                        // display or not connectede carplay deviceName
                                        this._displayDeviceNameEntCarPlayMenu(this._masterEntertainmentDataList.items[CPWifiIndex], !isDisabled, deviceName);
                                    }
                                    else
                                    {
                                        if (this._isCarPlayWifiMenu(this._masterEntertainmentDataList.items[CPWifiIndex]))
                                        {
                                            // display or not connectede carplay deviceName
                                            this._displayDeviceNameEntCarPlayMenu(this._masterEntertainmentDataList.items[CPWifiIndex], !isDisabled, deviceName);

                                            // if "Apple CarPlay" menu is visible and with device then USBCarPlay menu should be invisible
                                            isDisabled = true;
                                            dataList.items[i] = this._masterEntertainmentDataList.items[this._USB1EntIndex];
                                        }
                                        else
                                        {
                                            dataList.items[i] = this._masterEntertainmentDataList.items[CPIndex];
                                        }
                                    }
                                }
                                if ((dataList.items[i].appData.appName.indexOf(appName) === 0 && dataList.items[i].text1Id == 'USB1CarPlay') || updateListForCarPlay)
                                {
                                    if(this._AtSpeedDisabled)// At speed : Available status will be skipped for the speed restricted apps
                                    {
                                        var speedRestrictedAppName = null;
                                        ...
                                        for(var j = 0; j < this._SpeedRestrictedApps.length; ++j)
                                        {
                                            ...
                                            {
                                                log.debug("appName : " + dataList.items[i].appData.appName + " is not target.");
                                            }
                                        }
                                        if(isNoAppAtSpeed)
                                        {
                                            ...
                                            {
                                                log.debug("isDisabled : " + isDisabled + ". nothig to do.");
                                            }
                                            this._currentContextTemplate.list2Ctrl.updateItems(i, i);
                                        }
                                        else
                                        {
                                            log.debug("isNoAppAtSpeed : " + isNoAppAtSpeed + ". nothig to do.");
                                        }
                                    }
                                    else
                                    {
                                    ...
                                    log.debug("Updated current screen based on StatusMenu message: " + msg.params.payload.appName + " " + msg.params.payload.state);
                                }
                            }
                        }
                        else
                        {
                            log.error("_currentContextTemplate is NULL");
                        }
                        break;
                    default:
                        log.debug("context is not Entertainment. nothing to do.");
                        break;
                }
            }
            else
            {
                log.error("_currentContext is NNLL");
            }
        }
        else
        {
            log.warn("appName is not carplay. nothing to do.");
        }
    }
    else
    {
...
systemApp.prototype._displayDeviceNameEntCarPlayMenu = function(EntCarPlayItem, isAvailable, name)
{
	log.info("_displayDeviceName_EntCarPlayMenu  EntCarPlayItem=" + EntCarPlayItem + ", isAvailable=" + isAvailable + ", deviceName=" + name);

    if (isAvailable && typeof name === "string" && name.length > 0)
    {
        EntCarPlayItem.text1Id = "APDevice";
        EntCarPlayItem.text1SubMap = { "device name": name };
    }
    else
    {
        EntCarPlayItem.text1Id = "CarPlay";
        EntCarPlayItem.text1SubMap = null;
    }
    ...
    if (this._currentContext && this._currentContext.ctxtId === "Entertainment" && this._currentContextTemplate)
    {
        ...
        for (var i = 0; i < list.dataList.items.length; ++i)
        {
            if (list.dataList.items[i].appData.appName === 'carplay' && list.dataList.items[i].appData.mmuiEvent === 'SelectCarPlayWifi')
            {
                list.dataList.items[i] = EntCarPlayItem;
                list.updateItems(i, i);
                break;
            }
        }
...
systemApp.prototype._StatusMenuVisibleCarPlayWifiMsgHandler = function(msg)
{
    if (msg.params && msg.params.payload)
    {
        log.info("Received StatusMenuVisibleCarPlayWifi message: " + msg.params.payload.appName + " " + msg.params.payload.state);
        ...
        if (appName === "carplay")
        {
            ...
            for (var i = 0; i < this._masterEntertainmentDataList.items.length; ++i)
            {
                if (this._masterEntertainmentDataList.items[i].appData.mmuiEvent === mmuiEvent)
                {
                    ...
                    // if isVisible is true, Force Available "Apple Carplay" menu.
                    this._masterEntertainmentDataList.items[i].disabled = !isVisible;
                    break;
                }
                else
                {
...
systemApp.prototype._StatusMenuVisibleAndroidAutoMsgHandler = function(msg)
{
    if (msg.params && msg.params.payload)
    {
        log.info("Received StatusMenuVisibleAndroidAuto message: " + msg.params.payload.appName + " " + msg.params.payload.state);
        ...
        if (appName === "androidauto")
        {
            ...
            this._enableAppListItem(appName, isDisabled, this._masterEntertainmentDataList);
            // Update androidauto icon of HomeScreen
            this._setRemoteUIButton(appName, !isDisabled);
...
systemApp.prototype._enableEntertainmentListItemCarPlay = function(isDisabled, deviceName)
{
    var dataList = this._masterEntertainmentDataList;

    for (var i = 0; i < dataList.items.length; ++i)
    {
        if ((dataList.items[i].appData.appName.indexOf("carplay") === 0) &&
            dataList.items[i].text1Id == 'USB1CarPlay')
        {
            if (this._AtSpeedDisabled) // At speed : Available status will be skipped for the speed restricted apps
            {
                var speedRestrictedAppName = null;
            ...
            // Care about "Apple Carplay" menu
            var CPWifiIndex = i-1; //Entertainment List index of "Apple CarPlay"
            if (this._isCarPlayWifiMenu(dataList.items[CPWifiIndex]))
            {
                // display or not connectede carplay deviceName
                this._displayDeviceNameEntCarPlayMenu(dataList.items[CPWifiIndex], !isDisabled, deviceName);
                // if "Apple CarPlay" menu is visible and with device then USBCarPlay menu should be invisible
                dataList.items[i].appData.isVisible = false;
                dataList.items[this._USB1EntIndex].appData.isVisible = true;
            }
            else
            {
                // if carplay is available change state of USB1 (carPlay disabled -> USB enabled and vice-a-versa)
            ...
            log.debug("Updated cached list item based on StatusMenu message: carplay" + !isDisabled);
        }
    }
    ...
    {
        log.debug("VehicleConfigurationType is not changed.");
    }
};
